  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (response.status !== 200) {
        ...
        await CreateEvent(responJSON.cid);
      }
      // check response, if success is false, dont take them to success page
    } catch (error) {
      alert(`Oops! Something went wrong. Please refresh and tru again. Error ${error}`)
    }
  }
  ...
  const CreateEvent = async (cid) => {
    try {
      ...
      if (rsvpContract) {
        ...
        setSuccess(true);
        setLoading(false);
        setMessage("Your event has been cerated successfully.");
      } else {
